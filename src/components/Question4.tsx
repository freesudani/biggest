...
import { countriesActions5 } from "../redux/fetch5";
import { useNavigate } from "react-router-dom";
import ResultBox1 from "./ResultBox1";
import ResultBox2 from "./ResultBox2";

const Question1: FC = () => {
  const country = useAppSelector((state) => state.countries4);
  const dispatch = useAppDispatch();
  const navigate = useNavigate();

  const firstChoiseHandler = () => {
    dispatch(countriesActions4.chooseCountry1());
    dispatch(countriesActions5.setQuestion());
    setTimeout(() => navigate("/q5"), 2500);
  };

  const secondChoiseHandler = () => {
    dispatch(countriesActions4.chooseCountry2());
    dispatch(countriesActions5.setQuestion());
    setTimeout(() => navigate("/q5"), 2500);
  };
